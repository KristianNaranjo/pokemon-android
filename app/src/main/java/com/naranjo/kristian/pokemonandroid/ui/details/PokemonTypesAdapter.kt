...
class PokemonTypesAdapter : BaseListAdapter<TypeItem, Pokemon.Type>(TypeItem::type) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewTypeOrdinal: Int): BaseViewHolder<TypeItem> =
            when (val viewType = ViewType.values()[viewTypeOrdinal]) {
                ViewType.TYPE_ONLY -> PokemonTypeViewHolder(parent, viewType.layoutResId) as BaseViewHolder<TypeItem>
                ViewType.TYPE_EFFECTIVENESS -> PokemonTypeEffectivenessViewHolder(parent, viewType.layoutResId) as BaseViewHolder<TypeItem>
...
class PokemonTypeViewHolder(parent: ViewGroup, layoutResId: Int) :
    ...
    override fun bindData(data: TypeOnly) {
        ...
        this.type.background.setColorFilter(this.type.context.getColor(type.colorResId), PorterDuff.Mode.SRC_ATOP)
    }
}
...
data class TypeEffectivenessItem(override val type: Pokemon.Type, val effectiveness: Float) : TypeItem(type)
